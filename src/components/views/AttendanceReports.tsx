...
export default function AttendanceReports() {
  ...
  // Determine if user should see filtered classes
  const isFilteredRole = currentUser && currentUser.role !== 'admin' && currentUser.role !== 'guru_bk'

  useEffect(() => {
    // Fetch classes based on user role
    const classesUrl = isFilteredRole && currentUser
      ? `/api/classes?userId=${currentUser.id}`
      : '/api/classes'

    fetch(classesUrl).then(r => r.json()).then(d => {
      if (Array.isArray(d)) {
        setClasses(d)
        // Auto-select first class for teachers/wali_kelas
        if (d.length > 0 && isFilteredRole) {
          setClassId(d[0].id)
        }
      }
    })
        ...
        <div>
          ...
          {isFilteredRole && (
            <p className="text-xs text-muted-foreground mt-1">
              Menampilkan data kelas yang Anda ajar
            </p>
          )}
        </div>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="space-y-1 flex-1">
                <label className="text-xs font-medium text-muted-foreground">
                  {isFilteredRole ? 'Kelas yang Diajar' : 'Kelas'}
                </label>
                <Select value={classId} onValueChange={setClassId}>
                  <SelectTrigger><SelectValue placeholder={isFilteredRole ? 'Pilih kelas Anda' : 'Semua Kelas'} /></SelectTrigger>
                  <SelectContent>
                    {!isFilteredRole && (
